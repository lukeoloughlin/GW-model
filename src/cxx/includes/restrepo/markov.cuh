...
__device__ void update_RyR_rates(float* RyR_rates, const float cp, const float Ku, const float Kb, const float tau_u, 
                                 const float tau_b, const float tau_c, const float B0_CSQN, const float Mhat, const int linear_grid_idx);
...
__device__ void update_LCC_rates(float* LCC_rates, const float alpha, const float beta, const float s1, const float s2,
                                 ...
                                 const float r1, const float r2, const int linear_grid_idx);


__device__ void update_RyR_diffusion(float* RyR, float* RyR_rates, const int linear_grid_idx);
...
__device__ void update_LCC_exact(float* LCC, float* LCC_rates, const int linear_grid_idx);
