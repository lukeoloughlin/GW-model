...
struct PyGWOutput {
    ...
    PyGWOutput(int nCRU_, int num_step, double t_) : nCRU(nCRU), t(t_), V(num_step), m(num_step), h(num_step), j(num_step), 
                                                     ...
                                                     CaHTRPN(num_step), xKs(num_step), XKr(num_step,5), XKv14(10,num_step), XKv43(10,num_step), 
...
public:
    ...
    PyGWOutput run(double step_size, int num_steps, std::function<double(double)> &Is, int record_every){
