...
def INaCa(
    ...
    if lattice:
        return (
            multiplier
            * (
                (exp_term1 * (Nai**3))[:, np.newaxis, np.newaxis] * parameters.Cao
                - exp_term2[:, np.newaxis, np.newaxis] * (parameters.Nao**3) * Cai
            )
            / (1 + parameters.ksat * exp_term2[:, np.newaxis, np.newaxis])
        )
    else:
        return (
            multiplier
...
def ICab(
    ...
    """
    if lattice:
        return parameters.GCab * (V[:, np.newaxis, np.newaxis] - ECa(Cai, parameters))
    else:
