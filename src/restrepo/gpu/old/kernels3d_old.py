...
def update_currents_and_lcc_3d(
    ...
    if x < Nx and y < Ny and z < Nz:
        ...
        cjsr_tmp = cjsr[x, y, z]
        if task == 0:
            RyR_open[x, y, z] = max(RyR[x, y, z, 1] + RyR[x, y, z, 2], f32(0))
            # update RyR rates
            update_RyR_rates_3d(
                RyR_rates,
                RyR,
...
@cuda.jit
def update_currents_and_lcc_3d_rapid_cs(
    RyR: npt.NDArray[f32],
    LCC: npt.NDArray[i32],
    ci: npt.NDArray[f32],
    cs: npt.NDArray[f32],
    ...
    if x < Nx and y < Ny and z < Nz:
        ...
        if task == 0:
            ...
            update_diffusive_fluxes_3d_rapid_cs(
                Delta_ci,
                Delta_cnsr,
                sum_cs_nn,
                time_const_cs,
...
def update_RyR_and_conc_3d(
    ...
    if x < Nx and y < Ny and z < Nz:
        ...
        if task == 0:
            ...
            Ileak_ = Ileak(cnsr_tmp, ci_tmp, params.gleak[0], square(params.Knsr[0]))
            Iup_ = Iup(
                ci_tmp,
                cnsr_tmp,
                params.Ki[0],
            ...
            # update_RyR_diffusion_3d(
            #    RyR, RyR_sorted, RyR_rates, corrections, dW, dt, x, y, z
            # )
            update_RyR_diffusion_3d(RyR, RyR_rates, dW, dt, x, y, z)


@cuda.jit
def update_RyR_and_conc_3d_rapid_cs(
    ci: npt.NDArray[f32],
    cs: npt.NDArray[f32],
    cp: npt.NDArray[f32],
    cnsr: npt.NDArray[f32],
    ...
    if x < Nx and y < Ny and z < Nz:
        ...
        if task == 0:
            ...
            Ici = Delta_ci[x, y, z]
            Icnsr = Delta_cnsr[x, y, z]
            sum_cs_nn_ = sum_cs_nn[x, y, z]
            time_const_cs_ = time_const_cs[x, y, z]
            ...
            kr = params.Jmax[0] / vp_tmp
            ks = vp_tmp / (params.vs[0] * params.tau_ps[0])
            cp[x, y, z] = (
                cs_tmp + params.tau_ps[0] * (kr * ryr_open * cjsr_tmp - ICa_)
            ) / (float32(1.0) + params.tau_ps[0] * kr * ryr_open)
            cs[x, y, z] = (
                cp_tmp * ks + INaCa_ + ci_tmp / params.tau_si[0] - ITCs + sum_cs_nn_
            ) / (ks + f32(1) / params.tau_si[0] + time_const_cs_)
