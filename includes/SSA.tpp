...
namespace GW {
    ...
    void CRUStateThread<FloatType>::copy_from_CRUState(const CRUState<FloatType> &state, const Array2<FloatType> &JLCC, const int idx, const Parameters<FloatType> &params){
        ...
        memset(LCC_activation_rates, 0, 4*sizeof(FloatType));
    ...
    void update_rates(CRUStateThread<FloatType> &state, const Parameters<FloatType> &params, const Constants<FloatType> &consts){
        for (int j = 0; j < 4; j++){
            ...
            update_LCC_activation_rates(state.LCC_activation_rates, state.subunit_rates, state.LCC_activation, consts.yinfLCC, consts.tauLCC, j);
    ...
    void sample_RyR56(CRUStateThread<FloatType> &state, const int idx, const Parameters<FloatType> &params){
        ...
        if (n56 > 0){
            ...
            state.RyR[4+6*idx] = sample_binomial(p, n56);
    ...
    void sample_RyR34(CRUStateThread<FloatType> &state, const int idx, const Parameters<FloatType> &params){
        ...
        if (n34 > 0){
            ...
            state.RyR[2+6*idx] = sample_binomial(p, n34);
    ...
    void sample_LCC(CRUStateThread<FloatType> &state, const FloatType sum_LCC_rates, const int subunit_idx, const Constants<FloatType> &consts){
        const int transition = sample_weights<FloatType>(state.LCC_rates + 3*subunit_idx, sum_LCC_rates, 3); // using pointer arithmetic here
        ...
        case 5:
            if (transition == 0)
                ...
                state.JLCC[subunit_idx] = (state.LCC_activation[subunit_idx] == 1) ? consts.JLCC_multiplier * (consts.Cao_scaled - consts.JLCC_exp * state.CaSS[subunit_idx]) : 0; 
            }
            else 
        ...
        case 11:
            if (transition == 0)
                ...
                state.JLCC[subunit_idx] = (state.LCC_activation[subunit_idx] == 1) ? consts.JLCC_multiplier * (consts.Cao_scaled - consts.JLCC_exp * state.CaSS[subunit_idx]) : 0.0;
            }
            break;
        case 12:
            state.LCC[subunit_idx] = 11;
    ...
    inline void sample_RyR(CRUStateThread<FloatType> &state, const FloatType sum_RyR_rates, const int subunit_idx, const Parameters<FloatType> &params){
        const int transition = sample_weights(state.RyR_rates + 12*subunit_idx, sum_RyR_rates, 12); // using pointer arithmetic here
        switch (transition){
        case 0: // 1 -> 2
    ...
    void sample_new_state(CRUStateThread<FloatType> &state, const int subunit_idx, const Parameters<FloatType> &params, const Constants<FloatType> &consts){
        ...
        FloatType LCC_activation_rate = state.LCC_activation_rates[subunit_idx];
        ...
        if (u < sum_LCC_rates)
            ...
            state.LCC_activation[subunit_idx] = 1 - state.LCC_activation[subunit_idx];
            if (state.LCC_activation[subunit_idx] == 0)
                state.JLCC[subunit_idx] = 0;
        ...
        else if (u < (sum_LCC_rates + LCC_activation_rate + sum_RyR_rates))
            sample_RyR(state, sum_RyR_rates, subunit_idx, params);
        else 
    ...
    void SSA_single_CRU(CRUStateThread<FloatType> &state, const FloatType Cai, const FloatType CaNSR, const FloatType dt, const Parameters<FloatType> &params, const Constants<FloatType> &consts){
        ...
        while (1){
            ...
            jump_t = -log(urand<FloatType>()) / total_rate;
            if (t + jump_t < dt){
                update_CaSS(state, jump_t, params, consts);
                update_CaJSR(state, jump_t, params, consts);
            } 
            else {
                update_CaSS(state, dt-t, params, consts);
                update_CaJSR(state, dt-t, params, consts);
                break;
            }
            t += jump_t;
            
            subunit_idx = sample_weights<FloatType>(state.subunit_rates, total_rate, 4);
            sample_new_state(state, subunit_idx, params, consts);
        }
    }
}